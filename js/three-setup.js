document.addEventListener("DOMContentLoaded", () => {
  ...
  // === ANIMATION LOOP ===
  /**
   * Menghitung interpolasi linear (lerp) setiap frame untuk mendekati target posisi orbit kamera.
   * Loop ini akan terus meminta frame animasi berikutnya selama jarak selisih (dTheta/dPhi)
   * masih di atas nilai threshold. Hal ini memberikan efek rotasi kamera yang sangat halus
   * saat menangani gabungan input dari scroll dan pointer mouse.
   */
  function animationLoop() {
    ...
    if (dTheta > THRESHOLD || dPhi > THRESHOLD) {
      try {
        modelViewer.cameraOrbit = `${currentTheta.toFixed(2)}deg ${currentPhi.toFixed(2)}deg 40%`;
      } catch (_) {}
  ...
  modelWrapper.addEventListener("pointermove", handlePointerMove, {
    passive: true,
  });
  modelWrapper.addEventListener("pointerleave", handlePointerLeave, {
    passive: true,
  });
  ...
  setTimeout(() => {
    if (!didLoad)
      console.warn("Model did not load within 8s. Check path/server.");
  }, 8000);
});
